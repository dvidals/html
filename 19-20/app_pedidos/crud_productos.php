...
if(isset($_POST['cr'])){
  ...
  $sql="insert into productos (Nombre, Descripcion, Peso, Stock, CodCat, Foto) values (:nom, :des, :pes, :sto, :cod, :fot)";
  $resultado=$bd->prepare($sql);
  $resultado->execute(array(":nom"=>$nombre,":des"=>$descripcion,":pes"=>$peso,":sto"=>$stock,":cod"=>$codcat, ":fot"=>$foto));
...
<form id="fileuploads" action="datosImagen.php" method="POST" enctype="multipart/form-data"></form>
<form action="<?php echo $_SERVER['PHP_SELF'];?>" method="post">
  ...
  <table width="50%" border="0" align="center">
    ...
    foreach ($registros as $persona):?>
      ...
      <td class="bot"><a href="editar_productos.php?CodProd=<?php echo $persona->CodProd?> & nom=<?php echo $persona->Nombre?> & des=<?php echo $persona->Descripcion?> & pes=<?php echo $persona->Peso?>& sto=<?php echo $persona->Stock?>& cod=<?php echo $persona->CodCat?>& fot=<?php echo $persona->Foto?>"><input type='button' name='up' id='up' value='Actualizar'></a></td>
    </tr>       
...
?>
	...
	<td></td>
      ...
      <tr><td></td><td colspan=2>Página <?php
//----------------------PAGINACIÓN-----------------
  for($i=1;$i<=$paginas;$i++){
...
?>
</td><td colspan=4><div id="fileuploads">
<input type="file" name="imagen" form="fileuploads" size"20"><input type="submit" form="fileuploads" value="Enviar Archivo"></div></td></tr>    
  </table>
