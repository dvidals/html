...
<form  method="post">
    ...
    <div id="agenda">
       ...
       for ($i=0;$i<count($_POST["paco"]);$i++) {
            ...
            if ($nombre!=null) {     //para que vaya creciendo la agenda. 
                agenda_agregar($nombre, $telefono);//Cada vez que le damos a submit tenemos que volver a crear lo que ya teníamos en la agenda(nuevo 2019).
            }
        } 
        if (!$existe) {
                                  //si no existe en la agenda lo agregamos, no tendríamos agenda
            if (strlen($n)>0)
                agenda_agregar($n,$t);
        }
        ...
        function agenda_agregar($nombre,$telefono) {
            echo "<div id='datos'>
                    <td><p> $nombre</p></td>
                    <td><p>$telefono</p></td>
                    <input name='paco[]'  type='hidden' readonly value=$nombre>
                    <input name='pablo[]' type='hidden' readonly value=$telefono>
                    </div>";
        }
        ?>
    </div>
